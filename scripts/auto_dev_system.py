...
def audit_performance():
    ...
    if js_dir.exists():
        ...
        # New: Check for unminified JS files
        unminified_js = [f for f in js_files if not f.name.endswith('.min.js') and f.stat().st_size > 1024]  # >1KB
        report["checks"]["js_unminified_count"] = len(unminified_js)
        report["checks"]["js_unminified_files"] = [f.name for f in unminified_js]
    else:
        report["checks"]["js_files_count"] = 0
        report["checks"]["js_total_size"] = 0
        report["checks"]["js_unminified_count"] = 0
        report["checks"]["js_unminified_files"] = []
    ...
    if images_dir.exists():
        ...
        # New: Check for large images (over 500KB) that are not SVG
        large_images = [f for f in image_files if f.stat().st_size > 500 * 1024 and f.suffix != '.svg']
        report["checks"]["image_large_count"] = len(large_images)
        report["checks"]["image_large_files"] = [f.name for f in large_images]
    else:
        report["checks"]["image_files_count"] = 0
        report["checks"]["image_total_size"] = 0
        report["checks"]["image_large_count"] = 0
        report["checks"]["image_large_files"] = []
    ...
    # 7. 检查CSS资源
    css_dir = local_dir / 'assets' / 'css'
    if css_dir.exists():
        css_files = list(css_dir.rglob('*.css'))
        report["checks"]["css_files_count"] = len(css_files)
        report["checks"]["css_total_size"] = sum(f.stat().st_size for f in css_files)
        # New: Check for unminified CSS files
        unminified_css = [f for f in css_files if not f.name.endswith('.min.css') and f.stat().st_size > 1024]  # >1KB
        report["checks"]["css_unminified_count"] = len(unminified_css)
        report["checks"]["css_unminified_files"] = [f.name for f in unminified_css]
    else:
        report["checks"]["css_files_count"] = 0
        report["checks"]["css_total_size"] = 0
        report["checks"]["css_unminified_count"] = 0
        report["checks"]["css_unminified_files"] = []

    # 8. 检查HTML文件数量和大小
    html_files = list(local_dir.glob('*.html'))
    report["checks"]["html_files_count"] = len(html_files)
    total_html_size = sum(f.stat().st_size for f in html_files) if html_files else 0
    report["checks"]["html_total_size"] = total_html_size
    # New: Check for empty HTML files (like test.html, simple.html)
    empty_html_files = [f for f in html_files if f.stat().st_size < 50]  # Less than 50 bytes
    report["checks"]["html_empty_count"] = len(empty_html_files)
    report["checks"]["html_empty_files"] = [f.name for f in empty_html_files]
    ...
    # New penalties
    if report["checks"]["js_unminified_count"] > 0: score -= 5
    if report["checks"]["css_unminified_count"] > 0: score -= 5
    if report["checks"]["image_large_count"] > 0: score -= 10
    if report["checks"]["html_empty_count"] > 0: score -= 5
